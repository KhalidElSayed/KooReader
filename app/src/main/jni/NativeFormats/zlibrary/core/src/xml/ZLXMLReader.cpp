...
bool ZLXMLReader::readDecDocument(shared_ptr<ZLInputStream> stream) {
    ...
    do {
            ...
            if(bstart==false)
            {
                ...
                if(strstart)
                {
                   memcpy(pdata,strstart+strlen("<body>")+1,myParserBuffer+ length-strstart-strlen("<body>")-1);
                   plen=myParserBuffer+ length-strstart-strlen("<body>")-1;
                   bstart=true;
                }
            }
            else
            {
               char* strend=strstr(myParserBuffer,"</body>");
               if(strend)
               {
                  memcpy(pdata+plen,myParserBuffer,strend-1-myParserBuffer);
                  plen+=(strend-1-myParserBuffer);
                  break;
               }
               else
               {
